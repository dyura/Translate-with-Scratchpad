...
function onInstall(e) {
 ...
 function getFrequent() {
     ...
     var freq_prof=[];
    if (userProperties.getProperty("freq1")) {
     var i=1;
     while (userProperties.getProperty("freq"+i)) {
       freq_prof[i-1]=userProperties.getProperty("freq"+i);
       i++;
     }
   }
 ...
 function getTranslation(origin, dest, savePrefs, text) {
   if (savePrefs) {
     PropertiesService.getUserProperties()
         .setProperty('originLang', origin)
         .setProperty('destLang', dest);
 ...
 function addFreqProperties(start,arr) {
   var userProperties = PropertiesService.getUserProperties();
   for (i=0;i<arr.length;i++) {
       userProperties.setProperty('freq'+(start+i), arr[i])
   }
 }
